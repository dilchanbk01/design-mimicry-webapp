...
interface EditBannerDialogProps {
  ...
  onUpdate: (id: string, banner: Partial<HeroBanner>) => Promise<void>;
  banner: HeroBanner | null;
}

export function EditBannerDialog({ open, onOpenChange, onUpdate, banner }: EditBannerDialogProps) {
  ...
  // Load banner data when dialog opens or banner changes
  useEffect(() => {
    if (open && banner) {
      ...
      setPage(banner.page);
      if (banner.image_url) {
        setExistingImage(banner.image_url);
      }
    }
  }, [open, banner, setExistingImage]);
